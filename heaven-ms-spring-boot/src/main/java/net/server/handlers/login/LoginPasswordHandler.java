...
public final class LoginPasswordHandler implements MaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (!remoteHost.contentEquals("null")) {
            if (CommonConfig.config.server.useIpValidation) {    // thanks Alex-0000 (CanIGetaPR) for suggesting IP validation as a server flag
                if (remoteHost.startsWith("127.")) {
                    if (!CommonConfig.config.server.localServer) { // thanks Mills for noting HOST can also have a field named "localhost"
                        c.announce(MaplePacketCreator.getLoginFailed(13));  // cannot login as localhost if it's not a local server
                        return;
                    }
                } else {
                    if (CommonConfig.config.server.localServer) {
        ...
        if (CommonConfig.config.server.automaticRegister && loginok == 5) {
            try {
                ...
                ps.setString(2, CommonConfig.config.server.bcryptMigration ? BCrypt.hashpw(pwd, BCrypt.gensalt(12)) : hashpwSHA512(pwd));
        ...
        if (CommonConfig.config.server.bcryptMigration && (loginok <= -10)) { // -10 means migration to bcrypt, -23 means TOS wasn't accepted
            try {
                con = MysqlConnection.getConnection();
        ...
        try{
            con = MysqlConnection.getConnection();
            ps = con.prepareStatement("update accounts set ip = ? where name = ?");
            ps.setString(1, remoteHost);
            ps.setString(2, login);
            ps.executeUpdate();
        } catch (SQLException e) {
            System.out.println("更新ip地址出错");
        }  finally {
            disposeSql(con, ps);
        }
        if (loginok == 3) {
