...
public class MapleGuild {
    ...
    public static int getIncreaseGuildCost(int size) {
        int cost = CommonConfig.config.server.expandGuildBaseCost + Math.max(0, (size - 15) / 5) * CommonConfig.config.server.expandGuildTierCost;
        
        if (size > 30) {
            return Math.min(CommonConfig.config.server.expandGuildMaxCost, Math.max(cost, 5000000));
        } else {
            return cost;
        }
    }
