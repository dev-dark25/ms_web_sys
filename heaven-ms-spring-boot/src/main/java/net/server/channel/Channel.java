...
public final class Channel {
    ...
    private final List<MapleExpeditionType> expeditionType = new ArrayList<>();
    ...
    private final MonitoredReadLock merchReadLock = MonitoredReadLockFactory.createLock(merchantLock);
    private final MonitoredWriteLock merchWriteLock = MonitoredWriteLockFactory.createLock(merchantLock);

    private final MonitoredReentrantLock[] faceLock = new MonitoredReentrantLock[CommonConfig.config.server.channelLocks];
    ...
    public Channel(final int world, final int channel, long startTime) {
        ...
        try {
            ...
            port += (world * 100);
            ip = CommonConfig.config.server.host + ":" + port;
            ...
            expeditionType.addAll(Arrays.asList(MapleExpeditionType.values()));
            ...
            wse.sendMessageAll("    频道 " + getId() + ": 监听端口 " + port);
            System.out.println("    频道 " + getId() + ": 监听端口 " + port);
        } catch (Exception e) {
            System.out.println("创建频道失败");
        }
    }
    ...
    public final synchronized void shutdown() {
        World worldServer = getWorldServer();
        try {
            if (finishedShutdown) {
                return;
            }

            wse.sendMessageAll("正在关闭大区 " + worldServer.getFlag() + " - 频道 " + channel);
            System.out.println("正在关闭大区 " + worldServer.getFlag() + " - 频道 " + channel);
            ...
            wse.sendMessageAll("    频道 " + channel + " 关闭成功");
            System.out.println("    频道 " + channel + " 关闭成功");
        } catch (Exception e) {
            System.out.println("    频道 " + channel + " 关闭失败" + e.getMessage());
        }
    }
    ...
    private void emptyLocks() {
        for (int i = 0; i < CommonConfig.config.server.channelLocks; i++) {
            // 修复空指针，https://gitee.com/sleepnap/heaven-ms-nap/issues/I83PGS
            if (NapComUtils.isEmpty(faceLock)) {
                break;
            }
            if (null == faceLock[i]) {
                continue;
    ...
    public int getChannelCapacity() {
        return (int) (Math.ceil(((float) players.getAllCharacters().size() / CommonConfig.config.server.channelLoad) * 800));
    }
    ...
    private static String[] getEvents() {
        File eventFile = new File("scripts/event");
        if (!eventFile.exists()) {
            return new String[0];
        }
        File[] listFiles = eventFile.listFiles();
        if (NapComUtils.isEmpty(listFiles)) {
            return new String[0];
        }
    ...
    public int pushWeddingReservation(Integer weddingId, boolean cathedral, boolean premium, Integer groomId, Integer brideId) {
        ...
        try {
            ...
            int delay = CommonConfig.config.server.weddingReservationDelay - 1 - weddingReservationQueue.size();
    ...
    public void setOngoingWedding(final boolean cathedral, Boolean premium, Integer weddingId, Set<Integer> guests) {
        ...
        if (weddingId != null) {
            ...
            }, CommonConfig.config.server.weddingReservationTimeout * 60 * 1000);
    ...
    public static long getRelativeWeddingTicketExpireTime(int resSlot) {
        return (resSlot * CommonConfig.config.server.weddingReservationInterval * 60 * 1000);
    }
    ...
    public String getWeddingReservationTimeLeft(Integer weddingId) {
        ...
        try {
            ...
            return venue + " - " + getTimeLeft(ongoingStartTime + (resStatus * CommonConfig.config.server.weddingReservationInterval * 60 * 1000)) + " from now";
        } finally {
            lock.unlock();
        }
    }
