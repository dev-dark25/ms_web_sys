    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (chr.isChangingMaps() || chr.isBanned()) {
            if (chr.isChangingMaps()) {
                FilePrinter.printError(FilePrinter.PORTAL_STUCK + chr.getName() + ".txt", "玩家 " + chr.getName() + " 切换地图时卡住了。时间戳: " + Calendar.getInstance().getTime().toString() + " 上次访问的地图id: " + chr.getLastVisitedMapids());
            }
