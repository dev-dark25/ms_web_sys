    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        switch (operation) {
            ...
            case 4: { // invite
                ...
                if (invited != null) {
                    if(invited.getLevel() < 10 && (!CommonConfig.config.server.useParTyForStarters || player.getLevel() >= 10)) { //min requirement is level 10
                        c.announce(MaplePacketCreator.serverNotice(5, "The player you have invited does not meet the requirements."));
                        return;
                    }
                    if(CommonConfig.config.server.useParTyForStarters && invited.getLevel() >= 10 && player.getLevel() < 10) {    //trying to invite high level
                        c.announce(MaplePacketCreator.serverNotice(5, "The player you have invited does not meet the requirements."));
                        return;
                    }
