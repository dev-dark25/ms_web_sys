...
public final class EnterMTSHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if(!chr.isAlive() && CommonConfig.config.server.useBuybackSystem) {
            BuybackProcessor.processBuyback(c);
            c.announce(MaplePacketCreator.enableActions());
        } else {
            if (!CommonConfig.config.server.useMts) {
                openCenterScript(c);
                c.announce(MaplePacketCreator.enableActions());
                return;
            }
            ...
            if(chr.getEventInstance() != null) {
                c.announce(MaplePacketCreator.serverNotice(5, "您正处在活动事件当中，目前无法进入商城或者MTS。"));
                c.announce(MaplePacketCreator.enableActions());
                return;
            }
            
            if(MapleMiniDungeonInfo.isDungeonMap(chr.getMapId())) {
                c.announce(MaplePacketCreator.serverNotice(5, "您所在的地图，无法进行频道更换、进入商城或者MTS."));
                c.announce(MaplePacketCreator.enableActions());
                return;
            }
            
            if (FieldLimit.CANNOTMIGRATE.check(chr.getMap().getFieldLimit())) {
                chr.dropMessage(1, "当前地图无法进行.");
                c.announce(MaplePacketCreator.enableActions());
                return;
            }
