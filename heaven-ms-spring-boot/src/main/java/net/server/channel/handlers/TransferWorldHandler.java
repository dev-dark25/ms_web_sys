...
public final class TransferWorldHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if(!CommonConfig.config.server.allowCashShopWorldTransfer || Server.getInstance().getWorldsSize() <= 1) {
        ...
        try (Connection con = MysqlConnection.getConnection();
            ...
            while(rs.next()) {
                ...
                if(completedTimestamp == null) { //has pending world transfer
                    ...
                    return;
                } else if(completedTimestamp.getTime() + CommonConfig.config.server.worldTransferCoolDown > System.currentTimeMillis()) {
