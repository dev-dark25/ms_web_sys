        ...
        if (c.tryacquireClient()) { // thanks MedicOP for assisting on concurrency protection here
            try {
                ...
                */
                ...
                if (!player.isHidden()) {
                    if(player.isGM() && CommonConfig.config.server.useAutoHideGm) {
                ...
                if (newcomer) {
                    ...
                    */
                    if (player.isGM()){
                        Server.getInstance().broadcastGMMessage(c.getWorld(), MaplePacketCreator.earnTitleMessage((player.gmLevel() < 6 ? "GM " : "超级管理员 ") + player.getName() + " 已登录"));
                    }
                ...
                if(CommonConfig.config.server.useAddRatesByLevel == true) player.setPlayerRates();
                ...
                if (CommonConfig.config.server.useNpcScriptable) {
