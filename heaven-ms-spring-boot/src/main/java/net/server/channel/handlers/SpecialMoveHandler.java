        ...
        */
        ...
        if (effect.getCooldown() > 0) {
            if (chr.skillIsCooling(skillid)) {
                ...
                if(MapleStatEffect.isHerosWill(skillid) && CommonConfig.config.server.useFastReuseHeroWill) {
                    cooldownTime /= 60;
                }
        ...
        if (skillid == Hero.MONSTER_MAGNET || skillid == Paladin.MONSTER_MAGNET || skillid == DarkKnight.MONSTER_MAGNET) { // Monster Magnet
            ...
            for (int i = 0; i < num; i++) {
                ...
                /*
                fixme 当技能是英雄的磁石，这里客户端会闪退
                1.当success为1的时候无事发生，当success为0的时候，客户端必定闪退，这个success还是客户端自己传上来的
                2.已测试不管 MaplePacketCreator.catchMonster返回给客户端的是0 or 1 or 不返回，都不影响客户端要闪退这个结果
                3.空吸的情况下因为怪物数num为0，不会进入这串代码
                4.我个人怀疑不是服务端的问题，需要客户端排查 2024.02.29
                5.2024.03.01该问题在其他客户端上验证了，其他客户端不会闪退，应是083cn的bug
                 */
