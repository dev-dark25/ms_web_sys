...
public final class TakeDamageHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        //在dojo玩家不能使用锅，所以致命攻击也应该关闭
        if (is_deadly && GameConstants.isDojo(chr.getMap().getId()) && !CommonConfig.config.server.useDeadlyDojo) {
            damage = 0;
            mpattack = 0;
        }
        ...
        if (GameConstants.isDojo(map.getId())) {
            chr.setDojoEnergy(chr.getDojoEnergy() + CommonConfig.config.server.dojoEnergyDmg);
            c.announce(MaplePacketCreator.getEnergy("energy", chr.getDojoEnergy()));
        }

        for (MapleCharacter player : banishPlayers) {  // 冷静，如果这个列表不为空，那么确实存在攻击者，相信我：）
