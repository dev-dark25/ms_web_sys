...
public final class NPCTalkHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (currentServerTime() - c.getPlayer().getNpcCooldown() < CommonConfig.config.server.reopenNpcInterval) {
            c.announce(MaplePacketCreator.enableActions());
            return;
        }
        ...
        if (obj instanceof MapleNPC) {
            ...
            if (GameEnums.DebugLevel.isSimple()) {
                c.getPlayer().dropMessage(5, "打开的NPC: " + npc.getId());
            }
            ...
            if (npc.getId() == 9010009) {   //is duey
                ...
                if (npc.getId() >= 9100100 && npc.getId() <= 9100200) {
                    ...
                    if (!hasNpcScript) {
                        if (!npc.hasShop()) {
                            FilePrinter.printError(FilePrinter.NPC_UNCODED, "NPC " + npc.getName() + "(" + npc.getId() + ") 脚本未编写.");
                            return;
