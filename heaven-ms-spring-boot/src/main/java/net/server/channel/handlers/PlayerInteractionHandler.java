            ...
            if (mode == Action.CREATE.getCode()) {
                ...
                if (mode == Action.OPEN_STORE.getCode()) {
                    ...
                    if (!CashOperationHandler.checkBirthday(c, birthday)) { // birthday check here found thanks to lucasziron
                        c.announce(MaplePacketCreator.serverNotice(1, "请再次检查生日日期。"));
                        return;
                    }
                ...
                if (shop != null && shop.isOwner(chr)) {
                    if(CommonConfig.config.server.useErasePermitOnOpenShop) {
                        try {
                ...
                if (shop != null && shop.isOwner(chr)) {
                    ...
                    if (CommonConfig.config.server.useEnforceMerchantSave) {
