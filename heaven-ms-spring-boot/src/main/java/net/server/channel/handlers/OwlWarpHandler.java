...
public final class OwlWarpHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if(ownerid == c.getPlayer().getId()) {
            c.announce(MaplePacketCreator.serverNotice(1, "您不能访问自己的商店."));
            return;
        }
            ...
            if(ps.isOpen()) {
                if(GameConstants.isFreeMarketRoom(mapid)) {
                    if(ps.getChannel() == c.getChannel()) {
                        ...
                        c.announce(MaplePacketCreator.serverNotice(1, "该商店目前位于另一个频道。当前位置：频道 " + hm.getChannel() + ", '" + hm.getMap().getMapName() + "'."));
                    }
                } else {
                    c.announce(MaplePacketCreator.serverNotice(1, "该商店目前位于 自由市场 区域之外。当前位置：频道 " + hm.getChannel() + ", '" + hm.getMap().getMapName() + "'."));
                }
            } else {
                //c.announce(MaplePacketCreator.serverNotice(1, "That merchant has either been closed or is under maintenance."));
                c.announce(MaplePacketCreator.getOwlMessage(18));
            ...
            if(hm.isOpen()) {
                if(GameConstants.isFreeMarketRoom(mapid)) {
                    if(hm.getChannel() == c.getChannel()) {
                        ...
                        c.announce(MaplePacketCreator.serverNotice(1, "该商家目前位于另一个频道。当前位置：频道 " + hm.getChannel() + ", '" + hm.getMap().getMapName() + "'."));
                    }
                } else {
                    c.announce(MaplePacketCreator.serverNotice(1, "该商家目前位于 自由市场 区域之外。当前位置：频道 " + hm.getChannel() + ", '" + hm.getMap().getMapName() + "'."));
                }
            } else {
                //c.announce(MaplePacketCreator.serverNotice(1, "That merchant has either been closed or is under maintenance."));
                c.announce(MaplePacketCreator.getOwlMessage(18));
