...
public abstract class AbstractDealDamageHandler extends AbstractMaplePacketHandler {
    ...
    protected void applyAttack(AttackInfo attack, final MapleCharacter player, int attackCount) {
        // 兼容后续如果需要用参数控制，直接改这里取参数即可
        int banWarnedCount = 10;
        int banAttackedCount = 10;
        int banMaxAttackCount = 50;
        long curr = System.currentTimeMillis();
        // 统计1s内的攻击次数
        if (curr - player.getLastAttackTime() > 1000) {
            player.setLastAttackTime(curr);
            player.getAttackedCount().set(1);
        } else {
            player.getAttackedCount().incrementAndGet();
        }
        // 只统计一次避免重复统计
        if (player.getAttackedCount().get() == banAttackedCount + 1) {
            player.getWarnedAttackCount().incrementAndGet();
            if (player.getLastWarnedTime() == -1) {
                player.setLastWarnedTime(curr);
            } else if (player.getLastWarnedTime() - curr < 2000) {
                player.setLastWarnedTime(curr);
            } else {
                // 超过3s，则认为连续攻击不是开挂重置警告次数
                player.getWarnedAttackCount().set(0);
            }
        }
        // 如果警告次数大于10，或者1s内攻击大于50次，则进行警告或封禁
        if (player.getWarnedAttackCount().get() > banWarnedCount || player.getAttackedCount().get() > banMaxAttackCount) {
            player.sendPolice("攻击频率过快，已被管理员进行监测");
            if (CommonConfig.config.server.useAutoBan) {
                player.autoban("攻击频率过快，疑似开怪。每秒攻击次数大于等于：" + player.getAttackedCount().get());
            }
            return;
        }

        final MapleMap map = player.getMap();
        if (map.isOwnershipRestricted(player)) {//是否已注册
            return;
        }

        Skill theSkill = null;
        MapleStatEffect attackEffect = null;
        final int job = player.getJob().getId();//职业
        try {
            if (player.isBanned()) {//查看是否被封号
                return;
            }
            if (attack.skill != 0) {//攻击技能
                ...
                if (player.getMp() < attackEffect.getMpCon()) {//攻击魔法值够不够
                ...
                if (attack.skill != Cleric.HEAL) {//不是牧师治疗技能
                    if (player.isAlive()) {
                        //战神，强盗，奇袭者的能量
                        ...
                        } else if (attack.skill == DawnWarrior.FINAL_ATTACK || attack.skill == WindArcher.FINAL_ATTACK) {//草莓骑士，风灵使者 终极攻击？
                            // prevent cygnus FA refreshing
                            mobCount = 15;
                        } else if (attack.skill == NightWalker.POISON_BOMB) {// Poison Bomb 毒弹
            ...
            if (attack.skill == ChiefBandit.MESO_EXPLOSION) {//侠客的金钱炸弹
                int delay = 0;
...
//                                return channel == player.getClient().getChannel() && mapId == map.getId();
//                            }).findFirst();
//                        }
...
//                        }
                    ...
                    for (Integer eachd : onedList) {
                        ...
                        if (player.isCalcDamage()) {
                            if (eachd > player.getMaxDamage()) {
                                player.setMaxDamage(eachd);
                            }
                            if (eachd < player.getMinDamage() || 0 == player.getMinDamage()) {
                                player.setMinDamage(eachd);
                            }
                            player.setTotalDamage(player.getTotalDamage() + eachd);
                        }
                    }
                    ...
                    if (player.getBuffedValue(MapleBuffStat.PICKPOCKET) != null && (attack.skill == 0 || attack.skill == Rogue.DOUBLE_STAB || attack.skill == Bandit.SAVAGE_BLOW || attack.skill == ChiefBandit.ASSAULTER || attack.skill == ChiefBandit.BAND_OF_THIEVES || attack.skill == Shadower.ASSASSINATE || attack.skill == Shadower.TAUNT || attack.skill == Shadower.BOOMERANG_STEP)) {
                        ...
                        Skill steal = SkillFactory.getSkill(Bandit.STEAL);//窃取技能
                    ...
                    if (player.getBuffedValue(MapleBuffStat.HAMSTRING) != null) {//虚弱
                    ...
                    if (player.getBuffedValue(MapleBuffStat.SLOW) != null) {//减速
                    ...
                    if (player.getBuffedValue(MapleBuffStat.BLIND) != null) {//致盲
                    ...
                    } else if (job == 412 || job == 422 || job == 1411) {//隐士，侠盗，夜行者3
                    ...
                    } else if (job >= 311 && job <= 322) {//射手，神射手，弩弓手，游侠，箭神
                        if (!monster.isBoss()) {
                            Skill mortalBlow;//致命一击
                            if (job == 311 || job == 312) {// 射手，神射手
                                mortalBlow = SkillFactory.getSkill(Ranger.MORTAL_BLOW);
                            } else {
                                mortalBlow = SkillFactory.getSkill(Sniper.MORTAL_BLOW);
                            }

                            int skillLevel = player.getSkillLevel(mortalBlow);//致命一击等级
                            if (skillLevel > 0) {
                                MapleStatEffect mortal = mortalBlow.getEffect(skillLevel);//特效
                    ...
                    if (attack.skill != 0) {
                        if (attackEffect.getFixDamage() != -1) {
                            ...
                            if (attack.skill == threeSnailsId) {
                                if (CommonConfig.config.server.useUltraThreeSnails) {
    ...
    protected AttackInfo parseDamage(LittleEndianAccessor lea, MapleCharacter chr, boolean ranged, boolean magic) {
        ...
        System.out.println("攻击怪物数量 numAttacked is: " + ret.numAttacked);
        System.out.println("伤害次数 numDamage is: " + ret.numDamage);
        System.out.println("numAttackedAndDamage is: " + ret.numAttackedAndDamage);
        System.out.println("allDamage is: " + ret.allDamage);
        return ret;
    }
