...
public class MapleParty {
    ...
    public static boolean createParty(MapleCharacter player, boolean silentCheck) {
        ...
        if (party == null) {
            if (player.getLevel() < 10 && !CommonConfig.config.server.useParTyForStarters) {
                player.announce(MaplePacketCreator.partyStatusMessage(10));
                return false;
            } else if (player.getAriantColiseum() != null) {
                player.dropMessage(5, "您在参加阿里安特战斗竞技场时无法请求创建队伍");
                return false;
            }
    ...
    public static boolean joinParty(MapleCharacter player, int partyid, boolean silentCheck) {
        ...
        if (party == null) {
            ...
            if (party != null) {
                ...
                player.announce(MaplePacketCreator.serverNotice(5, "由于该队伍已解散，您无法加入。"));
            }
        } else {
            if (!silentCheck) {
                player.announce(MaplePacketCreator.serverNotice(5, "由于您已经在一个队伍中，因此无法加入另一个队伍。"));
            }
        }
