...
public abstract class BaseScheduler {
    ...
    private void runBaseSchedule() {
        ...
        try {
            if(registeredEntries.isEmpty()) {
                ...
                if(idleProcs >= CommonConfig.config.server.mobStatusMonitorLife) {
    ...
    protected void registerEntry(Object key, Runnable removalAction, long duration) {
        ...
        try {
            idleProcs = 0;
            if(schedulerTask == null) {
                schedulerTask = TimerManager.getInstance().register(monitorTask, CommonConfig.config.server.mobStatusMonitorProc, CommonConfig.config.server.mobStatusMonitorProc);
            }
