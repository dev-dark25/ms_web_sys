...
public class MapleMonsterAggroCoordinator {
    ...
    public void startAggroCoordinator() {
        ...
        try {
            ...
            }, CommonConfig.config.server.mobStatusAggroInterval, CommonConfig.config.server.mobStatusAggroInterval);
        } finally {
            idleLock.unlock();
        }
        
        int timeDelta = (int) Math.ceil((Server.getInstance().getCurrentTime() - lastStopTime) / CommonConfig.config.server.mobStatusAggroInterval);
        if (timeDelta > 0) {
    ...
    private static void updateEntryExpiration(PlayerAggroEntry pae) {
        pae.toNextUpdate = (int) Math.ceil((120000L / CommonConfig.config.server.mobStatusAggroInterval) / Math.pow(2, pae.expireStreak + pae.currentDamageInstances));
    }
            ...
            for (PlayerAggroEntry pae : mobAggroList) {
                ...
                if (chr != null) {
                    if (player.getId() == pae.cid) {
                        return true;
                    } else if (pae.updateStreak < CommonConfig.config.server.mobStatusAggroPersistence && chr.isAlive()) {  // verifies currently leading players activity
                        return false;
                    }
                }
            }
