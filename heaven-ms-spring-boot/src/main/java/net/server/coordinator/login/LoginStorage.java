...
public class LoginStorage {
    ...
    public boolean registerLogin(int accountId) {
        ...
        synchronized (accHist) {
            if (accHist.size() > CommonConfig.config.server.maxAccountLoginAttempt) {
                long blockExpiration = Server.getInstance().getCurrentTime() + CommonConfig.config.server.loginAttemptDuration;
                Collections.fill(accHist, blockExpiration);

                return false;
            }
            
            accHist.add(Server.getInstance().getCurrentTime() + CommonConfig.config.server.loginAttemptDuration);
            return true;
        }
    }
