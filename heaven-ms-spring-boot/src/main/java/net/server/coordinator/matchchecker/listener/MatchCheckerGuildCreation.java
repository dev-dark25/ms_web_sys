    ...
    public AbstractMatchCheckerListener getListener() {
        return new AbstractMatchCheckerListener() {
            ...
            public void onMatchAccepted(int leaderid, Set<MapleCharacter> matchPlayers, String message) {
                ...
                if (leader.getMeso() < CommonConfig.config.server.createGuildCost) {
                    leader.dropMessage(1, "You do not have " + GameConstants.numberWithCommas(CommonConfig.config.server.createGuildCost) + " mesos to create a Guild.");
                    broadcastGuildCreationDismiss(matchPlayers);
                    return;
                }
                ...
                leader.gainMeso(-CommonConfig.config.server.createGuildCost, true, false, true);
