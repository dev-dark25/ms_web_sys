...
public class MaplePacketDecoder extends CumulativeProtocolDecoder {
    ...
    protected boolean doDecode(IoSession session, IoBuffer in, ProtocolDecoderOutput out) throws Exception {
        ...
        if (in.remaining() >= decoderState.packetlength) {
            ...
            if (GameEnums.DebugLevel.isFull()) { // Atoot's idea: packet traffic log, applied using auto-identation thanks to lrenex
                ...
                if (packetLen <= 3000) {
                    ...
                    if (op == null) {
                        System.out.println("未知的封包:" + SendTo);
                    }
                } else {
                    System.out.println("解码错误 封包长度 > 3000 当前长度： " + packetLen);
