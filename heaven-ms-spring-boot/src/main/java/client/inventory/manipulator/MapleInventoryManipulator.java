...
public class MapleInventoryManipulator {
    ...
    private static boolean addByIdInternal(MapleClient c, MapleCharacter chr, MapleInventoryType type, MapleInventory inv,
                                           int itemId, short quantity, String owner, int petid, short flag, long expiration) {
    ...
    public static void move(MapleClient c, MapleInventoryType type, short src, short dst) {
        ...
        MapleInventory inv = player.getInventory(type);
        ...
        if (GameEnums.DebugLevel.isSimple()) {
            player.message("移动的物品id：" + source.getItemId());
        }
    }
    ...
    private static boolean isDisappearingItemDrop(Item it) {
        ...
        if (ii.isDropRestricted(it.getItemId())) {
            ...
            if (CommonConfig.config.server.useEnforceUnMerchAbleCash) {     // thanks Ari for noticing cash drops not available server-side
                return true;
            } else if (ItemConstants.isPet(it.getItemId()) && CommonConfig.config.server.useEnforceUnMerchAblePet) {
                return true;
            }
    ...
    private static boolean isDroppedItemRestricted(Item it) {
        return CommonConfig.config.server.useEraseUnTradeAbleDrop && it.isUntradeable();
    }
