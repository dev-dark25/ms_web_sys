...
import cn.nap.utils.accuracy.NapAcUtils;
...
public class Equip extends Item {
    ...
    private static int getStatModifier(boolean isAttribute) {
        ...
        if (CommonConfig.config.server.useEquipmentLevelUpPower) {
    ...
    private static int randomizeStatUpgrade(int top) {
        int limit = Math.min(top, CommonConfig.config.server.maxEquipmentLevelUpStatUp);
    ...
    public Pair<String, Pair<Boolean, Boolean>> gainStats(List<Pair<StatUpgrade, Integer>> stats) {
        ...
        String lvupStr = "";
        Integer statUp, maxStat = CommonConfig.config.server.maxEquipmentStat;
    ...
    private void gainLevel(MapleClient c) {
        ...
        if (!stats.isEmpty()) {
            if (CommonConfig.config.server.useEquipmentLevelUpSlots) {
        ...
        String lvupStr = "'" + MapleItemInformationProvider.getInstance().getName(this.getItemId()) + "' 现在已经升级到等级 " + itemLevel + "! ";
        String showStr = "#e'" + MapleItemInformationProvider.getInstance().getName(this.getItemId()) + "'#b 现在已经升级到 #e等级 #r" + itemLevel + "#k#b!";
    ...
    public synchronized void gainItemExp(MapleClient c, int gain) {  // Ronan's Equip Exp gain method
        ...
        int equipMaxLevel = Math.min(30, Math.max(ii.getEquipLevel(this.getItemId(), true), CommonConfig.config.server.useEquipmentLevelUp));
        ...
        float masteryModifier = (float) (CommonConfig.config.server.equipExpRate * ExpTable.getExpNeededForLevel(1)) / (float) normalizedMasteryExp(reqLevel);
        ...
        if (GameEnums.DebugLevel.isMore())
    ...
    private boolean reachedMaxLevel() {
        ...
        return itemLevel >= CommonConfig.config.server.useEquipmentLevelUp;
    }
    ...
    public String showEquipFeatures(MapleClient c) {
        ...
        String eqpInfo = reachedMaxLevel() ? " #e#r已达到最高等级#k#n" : (" 经验: #e#b" + (int) itemExp + "#k#n / " + ExpTable.getEquipExpNeededForLevel(itemLevel));
    ...
    public void randomEquipValue(int random) {
        str = getRandomShort(str, random);
        dex = getRandomShort(dex, random);
        _int = getRandomShort(_int, random);
        luk = getRandomShort(luk, random);
        hp = getRandomShort(hp, random);
        mp = getRandomShort(mp, random);
        watk = getRandomShort(watk, random);
        matk = getRandomShort(matk, random);
        wdef = getRandomShort(wdef, random);
        mdef = getRandomShort(mdef, random);
        acc = getRandomShort(acc, random);
        avoid = getRandomShort(avoid, random);
        hands = getRandomShort(hands, random);
        speed = getRandomShort(speed, random);
        jump = getRandomShort(jump, random);
        vicious = (short) (vicious + random);
    }

    private Short getRandomShort(Short value, int random) {
        if (value == null || value == 0 || random > Short.MAX_VALUE) {
            return value;
        }
        // 四舍五入
        short scale = (short) NapAcUtils.scale(Math.random() * (random * 2) - random);
        short ret = (short) (value + scale);
        return ret <= 0 ? 1 : ret;
    }
}