...
public class WhereaMiCommand extends Command {
    ...
    public void execute(MapleClient c, String[] params) {
        ...
        player.yellowMessage("此地图的玩家:");
        for (MapleCharacter chr : chars) {
            player.dropMessage(5, ">> " + chr.getName() + " - " + chr.getId() + " - id: " + chr.getObjectId());
        }

        if (!playernpcs.isEmpty()) {
            player.yellowMessage("此地图的玩家NPC:");
            for (MaplePlayerNPC pnpc : playernpcs) {
                player.dropMessage(5, ">> " + pnpc.getName() + " - Scriptid: " + pnpc.getScriptId() + " - id: " + pnpc.getObjectId());
            }
        }

        if (!npcs.isEmpty()) {
            player.yellowMessage("此地图的NPC:");
            for (MapleNPC npc : npcs) {
                player.dropMessage(5, ">> " + npc.getName() + " - " + npc.getId() + " - id: " + npc.getObjectId());
            }
        }

        if (!mobs.isEmpty()) {
            player.yellowMessage("此地图的怪物:");
            for (MapleMonster mob : mobs) {
                if (mob.isAlive()) {
                    player.dropMessage(5, ">> " + mob.getName() + " - " + mob.getId() + " - id: " + mob.getObjectId());
                }
            }
        }
    }
