...
public class AutoAddCommand extends Command {
    ...
    public void execute(MapleClient c, String[] params) {
        ...
        if (params.length > 1) {
            try {
                size = Math.min(Integer.parseInt(params[1]), player.getRemainingAp());
            } catch (NumberFormatException e) {
                player.dropMessage("请确认输入的点数是否正确");
                return;
            }
        } else {
            size = Math.min(player.getRemainingAp(), CommonConfig.config.server.maxAp - player.getStr());
//            player.yellowMessage("输入 @autoadd <str/dex/int/luk> <点数>，给力量/敏捷/智力/运气进行加点");
//            return;
        }
        ...
        if (!result) {
            player.dropMessage("总属性不能大于 " + CommonConfig.config.server.maxAp + " 或小于 4");
        }
    }
}
