...
public class ShowRatesCommand extends Command {
    ...
    public void execute(MapleClient c, String[] params) {
        ...
        showMsg += "玩家经验倍率: #k" + player.getRawExpRate() + "x#k" + "\r\n";
        double addExpRate = 0.0;
        if (player.getClient().getChannelServer().getHiredMerchants().containsKey(player.getId()) && CommonConfig.config.server.hiredMerchantExpRate > 0) {
            showMsg += "雇佣商店额外倍率: #k" + CommonConfig.config.server.hiredMerchantExpRate + "x#k" + "\r\n";
            addExpRate += CommonConfig.config.server.hiredMerchantExpRate;
        }
        if (player.getCouponExpRate() != 1) {
            showMsg += "Coupon EXP Rate: #k" + player.getCouponExpRate() + "x#k" + "\r\n";
        }
        showMsg += "最终经验倍率: #e#b" + (player.getExpRate() + addExpRate) + "x#k#n" + (player.hasNoviceExpRate() ? " - 新手倍率" : "") + "\r\n";
        ...
        showMsg += "最终金币倍率: #e#b" + player.getMesoRate() + "x#k#n" + "\r\n";
        ...
        showMsg += "最终掉落倍率: #e#b" + player.getDropRate() + "x#k#n" + "\r\n";

        showMsg += "\r\n" + "#eBOSS掉落倍率#n" + "\r\n";
        showMsg += "世界BOSS掉落倍率: #k" + c.getWorldServer().getBossDropRate() + "x#k" + "\r\n";
        showMsg += "玩家BOSS掉落倍率: #k" + player.getRawDropRate() + "x#k" + "\r\n";
        if (player.getCouponDropRate() != 1) {
            showMsg += "Coupon DROP Rate: #k" + player.getCouponDropRate() + "x#k" + "\r\n";
        }
        showMsg += "最终BOSS掉落倍率: #e#b" + player.getBossDropRate() + "x#k#n" + "\r\n";

        if (CommonConfig.config.server.useQuestRate) {
            showMsg += "\r\n" + "#e任务倍率#n" + "\r\n";
