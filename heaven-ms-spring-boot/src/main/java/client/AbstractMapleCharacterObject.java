...
public abstract class AbstractMapleCharacterObject extends AbstractAnimatedMapleMapObject {
    ...
    public void addMaxHP(int delta) { //NPCCM父类增加血量上限函数
        effLock.lock();
        statWlock.lock();
        try {
    ...
    private boolean assignStrDexIntLuk(Integer deltaStr, Integer deltaDex, Integer deltaInt, Integer deltaLuk) {
        ...
        try {
            ...
            if (newStr < 4 || newStr > CommonConfig.config.server.maxAp) {
                return false;
            }

            if (newDex < 4 || newDex > CommonConfig.config.server.maxAp) {
                return false;
            }

            if (newInt < 4 || newInt > CommonConfig.config.server.maxAp) {
                return false;
            }

            if (newLuk < 4 || newLuk > CommonConfig.config.server.maxAp) {
                return false;
            }
