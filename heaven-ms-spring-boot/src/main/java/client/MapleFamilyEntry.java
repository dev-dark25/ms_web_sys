...
public class MapleFamilyEntry {
    ...
    public boolean useEntitlement(MapleFamilyEntitlement entitlement) {
        ...
        try(Connection con = MysqlConnection.getConnection(); PreparedStatement ps = con.prepareStatement("INSERT INTO family_entitlement (entitlementid, charid, timestamp) VALUES (?, ?, ?)")) {
            ps.setInt(1, id);
    ...
    public boolean refundEntitlement(MapleFamilyEntitlement entitlement) {
        int id = entitlement.ordinal();
        try(Connection con = MysqlConnection.getConnection(); PreparedStatement ps = con.prepareStatement("DELETE FROM family_entitlement WHERE entitlementid = ? AND charid = ?")) {
            ps.setInt(1, id);
            ps.setInt(2, getChrId());
            ps.executeUpdate();
        } catch(SQLException e) {
