...
public class NewYearCardRecord {
    ...
    public static void startPendingNewYearCardRequests() {
        List<Map<String, Object>> selectList = MysqlConnection.select("SELECT * FROM newyear WHERE timereceived = 0 AND senderdiscard = 0");
        if (NapComUtils.isEmpty(selectList)) {
            return;
        }
        for (Map<String, Object> map : selectList) {
            NewYearCardRecord newYear = new NewYearCardRecord(NapMapUtils.getInteger(map, "senderid"),
                    NapMapUtils.getString(map, "sendername"), NapMapUtils.getInteger(map, "receiverid"),
                    NapMapUtils.getString(map, "receivername"), NapMapUtils.getString(map, "message"));
            newYear.setExtraNewYearCardRecord(NapMapUtils.getInteger(map, "id"), NapMapUtils.getBool(map, "senderdiscard"),
                    NapMapUtils.getBool(map, "receiverdiscard"), NapMapUtils.getBool(map, "received"),
                    NapMapUtils.getLong(map, "timesent"), NapMapUtils.getLong(map, "timereceived"));
            Server.getInstance().setNewYearCard(newYear);
