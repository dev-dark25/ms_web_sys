...
public class MapleClient {
    ...
    public boolean checkPin(String other) {
        if (!(CommonConfig.config.server.pinEnabled && !canBypassPin())) {
            return true;
        }
    ...
    public boolean checkPic(String other) {
        if (!(CommonConfig.config.server.picEnabled && !canBypassPic())) {
            return true;
        }
        ...
        try {
            ...
            if (rs.next()) {
                ...
                if (getLoginState() > LOGIN_NOTLOGGEDIN) { // already loggedin
                    ...
                    loginok = (tos == 0) ? (!CommonConfig.config.server.bcryptMigration ? 23 : -23) : (!CommonConfig.config.server.bcryptMigration ? 0 : -10); // migrate to bcrypt
                } else {
                    loggedIn = false;
                    loginok = 4;
                }
    ...
    public void updateLoginState(int newstate) {
        ...
        try {
            ...
            try (PreparedStatement ps = con.prepareStatement("UPDATE accounts SET loggedin = ?, lastlogin = ? WHERE id = ?")) {
                // 在这里使用sql currenTtime可能会中止登录，感谢Arnah指出这一点
        ...
        if (player != null && player.isLoggedin() && player.getClient() != null) {
            ...
            try {
                ...
                if (!this.serverTransition) {
                    ...
                    player.logOff();
                    if (CommonConfig.config.server.instantNameChange) player.doPendingNameChange();
                    clear();
                } else {
    ...
    public void checkChar(int accid) {  /// issue with multiple chars from same account login found by shavit, resinate
        if (!CommonConfig.config.server.useCharacterAccountCheck) {
            return;
        }
    ...
    public void announceDisableServerMessage() {
