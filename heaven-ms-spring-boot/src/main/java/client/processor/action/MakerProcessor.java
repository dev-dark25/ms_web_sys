...
public class MakerProcessor {
    ...
    private static boolean removeOddMakerReagents(int toCreate, Map<Integer, Short> reagentids) {
        ...
        boolean isWeapon = ItemConstants.isWeapon(toCreate) || CommonConfig.config.server.useMakerPermissiveAtkUp;  // thanks Vcoc for finding a case where a weapon wouldn't be counted as such due to a bounding on isWeapon
    ...
    private static boolean addBoostedMakerItem(MapleClient c, int itemid, int stimulantid, Map<Integer, Short> reagentids) {
        ...
        if(CommonConfig.config.server.useEnhancedCrafting == true) {
