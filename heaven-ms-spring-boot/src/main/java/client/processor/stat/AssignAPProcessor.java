...
public class AssignAPProcessor {
    ...
    public static void APAutoAssignAction(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        try {
            ...
            if(CommonConfig.config.server.useServerAutoAssigner) {
                ...
                switch(stance) {
                    case MAGICIAN:
                        ...
                        if(CommonConfig.config.server.useAutoAssignSecondaryCap && luk + chr.getLuk() > CAP) {
                    ...
                    case BOWMAN:
                        ...
                        if(CommonConfig.config.server.useAutoAssignSecondaryCap && str + chr.getStr() > CAP) {
                    ...
                    case THIEF:
                        ...
                        if(CommonConfig.config.server.useAutoAssignSecondaryCap && dex + chr.getDex() > CAP) {
                ...
                c.announce(MaplePacketCreator.serverNotice(1, "检测到更好的AP分配结果:\r\nSTR: +" + statGain[0] + "\r\nDEX: +" + statGain[1] + "\r\nINT: +" + statGain[3] + "\r\nLUK: +" + statGain[2]));
            } else {
    ...
    private static int gainStatByType(MapleStat type, int[] statGain, int gain, int statUpdate[]) {
        ...
        if (type.equals(MapleStat.STR)) {
            ...
            if (newVal > CommonConfig.config.server.maxAp) {
                statGain[0] += (gain - (newVal - CommonConfig.config.server.maxAp));
                statUpdate[0] = CommonConfig.config.server.maxAp;
            } else {
                statGain[0] += gain;
                statUpdate[0] = newVal;
            }
            ...
            if (newVal > CommonConfig.config.server.maxAp) {
                statGain[3] += (gain - (newVal - CommonConfig.config.server.maxAp));
                statUpdate[3] = CommonConfig.config.server.maxAp;
            } else {
                statGain[3] += gain;
                statUpdate[3] = newVal;
            }
            ...
            if (newVal > CommonConfig.config.server.maxAp) {
                statGain[2] += (gain - (newVal - CommonConfig.config.server.maxAp));
                statUpdate[2] = CommonConfig.config.server.maxAp;
            } else {
                statGain[2] += gain;
                statUpdate[2] = newVal;
            }
            ...
            if (newVal > CommonConfig.config.server.maxAp) {
                statGain[1] += (gain - (newVal - CommonConfig.config.server.maxAp));
                statUpdate[1] = CommonConfig.config.server.maxAp;
            } else {
                statGain[1] += gain;
                statUpdate[1] = newVal;
            }
        ...
        if (newVal > CommonConfig.config.server.maxAp) {
            return newVal - CommonConfig.config.server.maxAp;
        }
        return 0;
    }
        ...
        try {
            ...
            switch (APFrom) {
                ...
                case 512: // luk
                    ...
                    break;
                case 2048: // HP
                    if(CommonConfig.config.server.useEnforceHpMpSwap) {
                        if (APTo != 8192) {
