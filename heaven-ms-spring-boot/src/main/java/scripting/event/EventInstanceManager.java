...
public class EventInstanceManager {
    ...
    private Map<Integer, List<Integer>> collectionSet = new HashMap<>(CommonConfig.config.server.maxEventLevels);
    private Map<Integer, List<Integer>> collectionQty = new HashMap<>(CommonConfig.config.server.maxEventLevels);
    private Map<Integer, Integer> collectionExp = new HashMap<>(CommonConfig.config.server.maxEventLevels);
    ...
    public final void setEventRewards(int eventLevel, List<Object> rwds, List<Object> qtys, int expGiven) {
        ...
        if (eventLevel <= 0 || eventLevel > CommonConfig.config.server.maxEventLevels) return;
    ...
    public final void setEventCleared() {
        ...
        for (MapleCharacter chr : getPlayers()) {
            chr.awardQuestPoint(CommonConfig.config.server.questPointPerEventClear);
        }
