...
//                }
        ...
        if (quantity >= 0) {
            ...
            if (ItemConstants.getInventoryType(id).equals(MapleInventoryType.EQUIP)) {
                ...
                if (item != null) {
                    ...
                    if (CommonConfig.config.server.useEnhancedCrafting == true && c.getPlayer().getCS() == true) {
                        Equip eqp = (Equip) item;
                        if (!(c.getPlayer().isGM() && CommonConfig.config.server.usePerfectGmScroll)) {
    ...
    public void givePartyExp(String PQ, boolean instance) {
        ...
        for (MaplePartyCharacter member : party.getMembers()) {
            ...
            if (CommonConfig.config.server.pqBonusExpRate > 0 && System.currentTimeMillis() <= CommonConfig.config.server.eventEndTimestamp) {
                player.gainExp((int) (exp * CommonConfig.config.server.pqBonusExpRate), true, true);
            }
        }
    }
