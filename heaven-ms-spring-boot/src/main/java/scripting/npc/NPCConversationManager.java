...
.JS文件 ms对象定义类 JS文件中 通过cm对象调用的就是这里面或它父类的一些方法
 */
...
public class NPCConversationManager extends AbstractPlayerInteraction {
    ...
    public void addmaxhp(int a) {//增加最大生命值，这个没有具体代码是因为他的父类已经完成了实现，我们只需要调用就行了
        getPlayer().addMaxHP(a);
    }
    ...
    public boolean canSpawnPlayerNpc(int mapid) {
        ...
        return !CommonConfig.config.server.playerNpcAutoDeploy && chr.getLevel() >= chr.getMaxClassLevel() && !chr.isGM() && MaplePlayerNPC.canSpawnPlayerNpc(chr.getName(), mapid);
    }
    ...
    public void doGachapon() {
        ...
        sendNext("恭喜你获得了 #b#t" + item.getId() + "##k.");
    ...
    public void sendServerChatContent(final int type, final String msg) {
        World world = getClient().getChannelServer().getWorldServer();
        world.dropMessage(type, msg);
    }
    ...
    public boolean isUsingOldPqNpcStyle() {
        return CommonConfig.config.server.useOldGmsStyledPqNpcs && this.getPlayer().getParty() != null;
    }
    ...
    public String startAriantBattle(MapleExpeditionType expedType, int mapid) {
        ...
        if (!(playersSize >= exped.getMinSize() && playersSize <= exped.getMaxSize())) {
            return "请确保在 #r" + exped.getMinSize() + " ~ " + exped.getMaxSize() + " 玩家#k 在这个地图开始.";
        }

        MapleMap leaderMap = this.getMap();
        for (MapleCharacter mc : players) {
            if (mc.getMap() != leaderMap) {
                return "所有参赛的玩家都应该在这个区域开始比赛.";
            }

            if (mc.getParty() != null) {
                return "所有参赛玩家都不能参加比赛.";
            }

            int level = mc.getLevel();
            if (!(level >= expedType.getMinLevel() && level <= expedType.getMaxLevel())) {
                return "这个队伍里有超出等级范围的玩家。所有玩家必须在20~30#k之间处于#blevel才能开始战斗";
            }
        }
    ...
    public boolean isBindKook() {
        return NapComUtils.isNotEmpty(KookMessageDao.selectBindResult(getPlayer().getAccountID()));
    }

    public boolean doBindKook(String bindCode) {
        KookMessage condition = new KookMessage();
        condition.setBindCode(bindCode);
        List<KookMessage> kookMessageList = KookMessageDao.selectByCondition(condition);
        if (NapComUtils.isEmpty(kookMessageList)) {
            return false;
        }
        KookMessage kookMessage = kookMessageList.get(0);
        condition = new KookMessage();
        condition.setAccountId(getPlayer().getAccountID());
        condition.setId(kookMessage.getId());
        if (kookMessage.getAccountId() == null || kookMessage.getAccountId()== 0) {
            KookMessageDao.update(condition);
            return true;
        }
        return kookMessage.getAccountId() == getPlayer().getAccountID();
    }
}