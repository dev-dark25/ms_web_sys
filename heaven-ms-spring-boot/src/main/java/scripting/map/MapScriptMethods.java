...
public class MapScriptMethods extends AbstractPlayerInteraction {
    ...
    public void explorerQuest(short questid, String questName) {
        ...
        if (status.equals(infoex)) {
            etm.append("获得 ").append(questName).append(" 称号！");
            smp.append("你已获得 <").append(questName).append(">").append(rewardstring);
            getPlayer().announce(MaplePacketCreator.getShowQuestCompletion(quest.getId()));
        } else {
            getPlayer().announce(MaplePacketCreator.earnTitleMessage(status + "/" + infoex + " 探索的区域。"));
            etm.append("正在进行 ").append(questName).append(" 称号。");
            smp.append("称号 ").append(questName).append(" 获取进度: ").append(status).append("/").append(infoex);
        }
    ...
    public void touchTheSky() { //29004
        ...
        String status = Integer.toString(qs.getMedalProgress());
        getPlayer().announceUpdateQuest(DelayedQuestUpdate.UPDATE, qs, true);
        getPlayer().announce(MaplePacketCreator.earnTitleMessage(status + "/5 已完成"));
        getPlayer().announce(MaplePacketCreator.earnTitleMessage("接触天空的人称号正在进行中"));
        if (Integer.toString(qs.getMedalProgress()).equals(qs.getInfoEx(0))) {
            showInfoText("接触天空的人" + rewardstring);
            getPlayer().announce(MaplePacketCreator.getShowQuestCompletion(quest.getId()));
        } else {
            showInfoText("接触天空的人称号正在进行中， " + status + "/5 已完成");
        }
    }
}
