...
public class MysqlConnection {
    private static final AtomicBoolean initialize = new AtomicBoolean();
    public static final String SOURCE_MYSQL = "napMs_mysql";
    ...
    public static void init() {
        ...
        dbProp.setUrl(CommonConfig.config.server.dbUrl);
        dbProp.setDriver(NapDriverEnum.MYSQL);
        dbProp.setUser(CommonConfig.config.server.dbUser);
        dbProp.setPassword(CommonConfig.config.server.dbPass);
        dbProp.setMaxConnect(CommonConfig.config.server.dbMaxConnect);
        // 这里单位为秒
...
//    }
    ...
    public static void closeConnection(Connection... connections) {
        if (null == connections || connections.length == 0) {
            NapDbSource.getInstance().close(SOURCE_MYSQL);
            return;
        }
        for (Connection connection : connections) {
