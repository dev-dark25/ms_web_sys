...
public class MapleStatEffect {
    ...
    private static MapleStatEffect loadFromData(MapleData source, int sourceid, boolean skill, boolean overTime) {
        ...
        if (ret.overTime && ret.getSummonMovementType() == null) {
            if (!skill) {
                ...
                } else if ((sourceid == Beginner.NIMBLE_FEET || sourceid == Noblesse.NIMBLE_FEET || sourceid == Evan.NIMBLE_FEET || sourceid == Legend.AGILE_BODY) && CommonConfig.config.server.useUltraNimbleFeet == true) {
        ...
        if (ltd != null) {
            ...
            if (CommonConfig.config.server.useMaxRangeEchoOfHero && (sourceid == Beginner.ECHO_OF_HERO || sourceid == Noblesse.ECHO_OF_HERO || sourceid == Legend.ECHO_OF_HERO || sourceid == Evan.ECHO_OF_HERO)) {
        ...
        if ((sourceid == Beginner.RECOVERY || sourceid == Noblesse.RECOVERY || sourceid == Evan.RECOVERY || sourceid == Legend.RECOVERY) && CommonConfig.config.server.useUltraRecovery == true) {
            x *= 10;
        }
        ret.x = x;
    ...
    public int getBuffLocalDuration() {
        return !CommonConfig.config.server.useBuffEverlasting ? duration : Integer.MAX_VALUE;
    }
