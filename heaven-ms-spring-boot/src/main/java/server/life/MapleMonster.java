...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    private void applyDamage(MapleCharacter from, int damage, boolean stayAlive, boolean fake) {
        ...
        if (GameEnums.DebugLevel.isFull()) {
            from.dropMessage(5, "¹¥»÷¹ÖÎï,ID: " + this.getId() + ", OID: " + this.getObjectId());
        }
    ...
    private void distributePlayerExperience(MapleCharacter chr, float exp, float partyBonusMod, int totalPartyLevel, boolean highestPartyDamager, boolean whiteExpGain, boolean hasPartySharers) {
        float playerExp = (CommonConfig.config.server.expSplitCommonMod * chr.getLevel()) / totalPartyLevel;
        if (highestPartyDamager) playerExp += CommonConfig.config.server.expSplitMvpMod;
    ...
    private void distributePartyExperience(Map<MapleCharacter, Long> partyParticipation, float expPerDmg, Set<MapleCharacter> underleveled, Map<Integer, Float> personalRatio, double sdevRatio) {
        ...
        leechInterval.addInterval(this.getLevel() - CommonConfig.config.server.expSplitLevelInterval, this.getLevel() + CommonConfig.config.server.expSplitLevelInterval);
        ...
        for (Entry<MapleCharacter, Long> e : partyParticipation.entrySet()) {
            ...
            leechInterval.addInterval(chrLevel - CommonConfig.config.server.expSplitLevelInterval, chrLevel + CommonConfig.config.server.expSplitLevelInterval);
        }
        ...
        if (CommonConfig.config.server.useEnforceMobLevelRange) {
    ...
    private float getStatusExpMultiplier(MapleCharacter attacker, boolean hasPartySharers) {
        ...
        if (holySymbol != null) {
            if (CommonConfig.config.server.useFullHolySymbol) { // thanks Mordred, xinyifly, AyumiLove, andy33 for noticing HS hands out 20% of its potential on less than 3 players
    ...
    private void giveExpToCharacter(MapleCharacter attacker, Float personalExp, Float partyExp, boolean white, boolean hasPartySharers) {
        if (attacker.isAlive()) {
            ...
            if (partyExp != null) {
                ...
                partyExp *= CommonConfig.config.server.partyBonusExpRate;
            } else {
                partyExp = 0.0f;
            }
    ...
    private void giveFamilyRep(MapleFamilyEntry entry) {
        if(entry != null) {
            int repGain = isBoss() ? CommonConfig.config.server.familyRepPerBossKill : CommonConfig.config.server.familyRepPerKill;
    ...
    public void debuffMob(int skillid) {
        ...
        try {
            if(skillid == Hermit.SHADOW_MESO) {
                ...
                if(CommonConfig.config.server.useAntiImmunityCrash) {
    ...
    public void aggroAutoAggroUpdate(MapleCharacter player) {
        ...
        if (chrController == null) {
            ...
            if (!CommonConfig.config.server.useAutoAggroNearby) {   // thanks Lichtmager for noticing autoaggro not updating the player properly
    ...
    public void aggroUpdatePuppetVisibility() {
        ...
        service.registerOverallAction(this.getMap().getId(), r, CommonConfig.config.server.commonRefresh);
    }
