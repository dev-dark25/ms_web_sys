...
public class MaplePlayerNPCPositioner {
    ...
    private static int calcDx(int newStep) {
        return CommonConfig.config.server.playerNpcAreaX / (newStep + 1);
    }
    
    private static int calcDy(int newStep) {
        return (CommonConfig.config.server.playerNpcAreaY / 2) + (CommonConfig.config.server.playerNpcAreaY / (1 << (newStep + 1)));
    }
    ...
    private static List<Point> rearrangePlayerNpcPositions(MapleMap map, int newStep, int pnpcsSize) {
        ...
        int leftPx = mapArea.x + CommonConfig.config.server.playerNpcInitialX, px, py = mapArea.y + CommonConfig.config.server.playerNpcInitialY;
        int outx = mapArea.x + mapArea.width - CommonConfig.config.server.playerNpcInitialX, outy = mapArea.y + mapArea.height;
    ...
    private static Point reorganizePlayerNpcs(MapleMap map, int newStep, List<MapleMapObject> mmoList) {
        if(!mmoList.isEmpty()) {
            if (GameEnums.DebugLevel.isMore()) {
                System.out.println("重新组织玩家NPC地图，步骤 " + newStep);
            }
    ...
    private static Point getNextPlayerNpcPosition(MapleMap map, int initStep) {   // automated playernpc position thanks to Ronan
        ...
        int i = initStep;
        while(i < CommonConfig.config.server.playerNpcAreaSteps) {
            int leftPx = mapArea.x + CommonConfig.config.server.playerNpcInitialX, px, py = mapArea.y + CommonConfig.config.server.playerNpcInitialY;
                ...
                while(px < outx) {
                    ...
                    if(searchPos != null) {
                        if(!isPlayerNpcNearby(otherPlayerNpcs, searchPos, cx, cy)) {
                            ...
                            if(reorganize && CommonConfig.config.server.playerNpcOrganizeArea) {
            ...
            cx = calcDx(i);
            cy = calcDy(i);
            if(CommonConfig.config.server.playerNpcOrganizeArea) {
                otherPlayerNpcs = rearrangePlayerNpcPositions(map, i, mmoList.size());
            }
        }
        
        if(i > initStep) {
            map.getWorldServer().setPlayerNpcMapStep(map.getId(), CommonConfig.config.server.playerNpcAreaSteps - 1);
        }
        return null;
    }
