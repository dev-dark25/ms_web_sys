...
public class MapleItemInformationProvider {
    ...
    protected MapleData specialData;
    ...
    private MapleItemInformationProvider() {
        ...
        specialData = itemData.getData("Special/0910.img");
    ...
    private MapleData getEqpData(int itemId) {
        String cat = "null";
        MapleData theData;
        if (itemId >= 5010000) {
        ...
        if (cat.equalsIgnoreCase("null")) {
            MapleData resultMapleData = theData.getChildByPath(String.valueOf(itemId));
            if (itemId > 5010000 && resultMapleData == null) {
                // 查找礼包名称
                theData = specialData;
                resultMapleData = theData.getChildByPath(String.valueOf(itemId));
            }
            return resultMapleData;
        } else {
    ...
    public static boolean rollSuccessChance(double propPercent) {
        return Math.random() >= testYourLuck(propPercent / 100.0, CommonConfig.config.server.scrollChanceRolls);
    }
    ...
    public void scrollOptionEquipWithChaos(Equip nEquip, int range, boolean option) {
        ...
        if (!option) {
            if (nEquip.getStr() > 0) {
                if (CommonConfig.config.server.useEnhancedChScroll)
    ...
    private void scrollEquipWithChaos(Equip nEquip, int range) {
        if (CommonConfig.config.server.chScrollStatRate > 0) {
            int temp;
            ...
            for (int i = 0; i < CommonConfig.config.server.chScrollStatRate; i++) {
                if (nEquip.getStr() > 0) {
                    if (CommonConfig.config.server.useEnhancedChScroll)
    ...
    */
    public boolean canUseCleanSlate(Equip nEquip) {
        ...
        return CommonConfig.config.server.useEnhancedClnSlate || nEquip.getUpgradeSlots() < (byte) (eqstats.get("tuc") + nEquip.getVicious());
    }

    public Item scrollEquipWithId(Item equip, int scrollId, boolean usingWhiteScroll, int vegaItemId, boolean isGM) {
        boolean assertGM = (isGM && CommonConfig.config.server.usePerfectGmScroll);
            ...
            if (((nEquip.getUpgradeSlots() > 0 || ItemConstants.isCleanSlate(scrollId))) || assertGM) {
                ...
                if (assertGM || rollSuccessChance(prop)) {
                    ...
                    switch (scrollId) {
                        ...
                        case 2049101:
                        case 2049102:
                            scrollEquipWithChaos(nEquip, CommonConfig.config.server.chScrollStatRange);
                            break;
                    ...
                    if (!CommonConfig.config.server.usePerfectScrolling && !usingWhiteScroll && !ItemConstants.isCleanSlate(scrollId) && !assertGM && !ItemConstants.isModifierScroll(scrollId)) {
                        nEquip.setUpgradeSlots((byte) (nEquip.getUpgradeSlots() - 1));
                    }
    ...
    public Equip getNullableEquip(int equipId) {
        Map<String, Integer> stats = getEquipStats(equipId);
        if (stats == null) {
            return null;
        }
        return (Equip) getEquipById(equipId, -1);
    }
    ...
    private static short getRandStat(short defaultValue, int maxRange) {
        ...
        // 先将默认值*0.1和最大范围取最小值，如力量30 * 0.1 = 3，和5取最小值，即3
        int lMaxRange = (int) Math.min(Math.ceil(defaultValue * 0.1), maxRange);
        /*
         再计算默认值 + (随机数) * (上面值 * 2 + 1)向下取整 - 上面值
         如力量30 + (0.0~1.0) * (3 * 2 + 1)向下取整 - 3
         即30 + (0~1) * 7向下取整 - 3
         即30 + (0~7)向下取整 - 3
         即30 + (0~7) - 3，即27~37
         */
    ...
    public Equip randomizeStats(Equip equip) {
        // 如果随机有等级限制，那么不能在drop的时候随机，只能在捡起的时候随机
        if (CommonConfig.config.server.pickedEquipRandomOpenLevel > 0) {
            return equip;
        }
        equip.randomEquipValue(CommonConfig.config.server.pickedEquipRandomValue);
//        equip.setStr(getRandStat(equip.getStr(), randomValue));
//        equip.setDex(getRandStat(equip.getDex(), randomValue));
//        equip.setInt(getRandStat(equip.getInt(), randomValue));
//        equip.setLuk(getRandStat(equip.getLuk(), randomValue));
//        equip.setMatk(getRandStat(equip.getMatk(), randomValue));
//        equip.setWatk(getRandStat(equip.getWatk(), randomValue));
//        equip.setAcc(getRandStat(equip.getAcc(), randomValue));
//        equip.setAvoid(getRandStat(equip.getAvoid(), randomValue));
//        equip.setJump(getRandStat(equip.getJump(), randomValue));
//        equip.setSpeed(getRandStat(equip.getSpeed(), randomValue));
//        equip.setWdef(getRandStat(equip.getWdef(), randomValue));
//        equip.setMdef(getRandStat(equip.getMdef(), randomValue));
//        equip.setHp(getRandStat(equip.getHp(), randomValue));
//        equip.setMp(getRandStat(equip.getMp(), randomValue));
        return equip;
    }
    ...
    public Pair<String, String> getNameDesc(int itemId) {
        ...
        MapleData strings = getEqpData(itemId);
        if (strings == null) {
            return null;
        }
        String name = MapleDataTool.getString("name", strings, null);
        String desc = MapleDataTool.getString("desc", strings, null);
        Pair<String, String> pair = new Pair<>(name, desc);
        nameDescCache.put(itemId, pair);
        return pair;
    }
    ...
    public boolean isUnmerchable(int itemId) {
        if (CommonConfig.config.server.useEnforceUnMerchAbleCash && isCash(itemId)) {
            return true;
        }

        if (CommonConfig.config.server.useEnforceUnMerchAblePet && ItemConstants.isPet(itemId)) {
            return true;
        }
    ...
    public boolean canWearEquipment(MapleCharacter chr, Equip equip, int dst) {
        ...
        if (ItemConstants.isWeddingRing(id) && chr.hasJustMarried()) {
            chr.dropMessage(5, "当前地图不能装备结婚戒指");  // will dc everyone due to doubled couple effect
            return false;
        }

        String islot = getEquipmentSlot(id);
        if (islot == null) {
            System.out.println("wz在服务端不存在，请检查服务的wz是否完整。物品ID：" + id);
            return false;
        }
    ...
    public MapleData getEqpData() {
        return eqpStringData;
    }
}
