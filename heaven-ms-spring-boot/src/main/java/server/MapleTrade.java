...
public class MapleTrade {
    ...
    private void completeTrade() {
        byte result;
        boolean show = GameEnums.DebugLevel.isMore();
        items.clear();
        meso = 0;
        ...
        if (exchangeMeso > 0) {
            ...
            if(fee > 0) {
                chr.dropMessage(1, "交易完成，你收到了 " + GameConstants.numberWithCommas(exchangeMeso - fee) + " 金币（减免后）");
            } else {
                chr.dropMessage(1, "交易完成，你收到了 " + GameConstants.numberWithCommas(exchangeMeso) + " 金币");
            }
    ...
    public void setMeso(int meso) {
        if (locked.get()) {
            throw new RuntimeException("交易已被锁定");
        }
        if (meso < 0) {
            Log.warn("[警告] " + chr.getName() + " 尝试交易的金额小于0");
            return;
        }
        if (chr.getMeso() >= meso) {
