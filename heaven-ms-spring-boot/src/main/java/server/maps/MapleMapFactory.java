...
public class MapleMapFactory {
    ...
    private static void loadLifeRaw(MapleMap map, int id, String type, int cy, int f, int fh, int rx0, int rx1, int x, int y, int hide, int mobTime, int team) {
        ...
        if (myLife instanceof MapleMonster) {
            ...
            //多倍怪,如果是BOSS,默认1倍，如果是小怪最高10倍,小于等于0默认1倍怪,大于10倍默认10倍怪
            int mobCount = 1;
            if (monster.isBoss()) {
                mobCount = 1;
            } else {
                mobCount = CommonConfig.config.server.bmMobRate;
                if(mobCount <= 0){
                   mobCount = 1;
                }
                if(mobCount > 10){
    ...
    public static MapleMap loadMapFromWz(int mapid, int world, int channel, EventInstanceManager event) {
        ...
        MapleData mapData = mapSource.getData(mapName);    // source.getData issue with giving nulls in rare occasions found thanks to MedicOP
