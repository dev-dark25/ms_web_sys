...
public class MapleMap {
    ...
    private long mapOwnerFirstActivityTime = Long.MAX_VALUE;
    ...
    private static double getRangedDistance() {
        return(CommonConfig.config.server.useMaxRange ? Double.POSITIVE_INFINITY : 722500);
    }
    ...
    private void dropFromMonster(final MapleCharacter chr, final MapleMonster mob, final boolean useBaseRate) {
        ...
        List<MonsterDropEntry> lootEntry = CommonConfig.config.server.useSpawnRelevantLoot ? mob.retrieveRelevantDrops() : mi.retrieveEffectiveDrop(mob.getId());
    ...
    private void stopItemMonitor() {
        ...
        if(CommonConfig.config.server.useSpawnLootOnAnimation) {
    ...
    private void startItemMonitor() {
        ...
        try {
            ...
            }, CommonConfig.config.server.itemMonitorTime, CommonConfig.config.server.itemMonitorTime);
            ...
            }, CommonConfig.config.server.itemExpireCheck, CommonConfig.config.server.itemExpireCheck);
    ...
    private void instantiateItemDrop(MapleMapItem mdrop) {
        if(droppedItemCount.get() >= CommonConfig.config.server.itemLimitOnMap) {
    ...
    private void registerItemDrop(MapleMapItem mdrop) {
        droppedItems.put(mdrop, !everlast ? Server.getInstance().getCurrentTime() + CommonConfig.config.server.itemExpireTime : Long.MAX_VALUE);
    }
...
//                设置地图所有者
    ...
    public void spawnKite(final MapleKite kite) {
        ...
        getWorldServer().registerTimedMapObject(expireKite, CommonConfig.config.server.kiteExpireTime);
    }
    ...
    */
        ...
        if (monster.isBoss()) {
            if (unclaimOwnership() != null) {
                ...
                if (mobName != null) {
                    ...
                    this.dropMessage(5, "这里已被" + mobName + "的部队包围，并将一直坚守到他们被打败为止。");
                }
            }
        }
    }
    ...
    private int getNumShouldSpawn(int numPlayers) {
        ...
        */
        ...
        if(CommonConfig.config.server.useEnableFullRespawn) {
    ...
    public boolean claimOwnership(MapleCharacter chr) {
        if (mapOwner == null) {
            ...
            mapOwnerFirstActivityTime = Server.getInstance().getCurrentTime();
    ...
    //    获取归属者攻击时间
    public String getMapOwnerAttackTime() {
        long timeNow = Server.getInstance().getCurrentTime();
        long milliseconds = timeNow - mapOwnerFirstActivityTime;
        ...
        return (days == 0 ? "" : days + "天") + (hours == 0 ? "" : hours + "小时") + (minutes == 0 ? "" : minutes + "分钟") + (seconds == 0 ? "1秒" : seconds + "秒");
    }
...
//                return true;
    ...
    public void checkMapOwnerActivity() {
        ...
        if (timeNow - mapOwnerLastActivityTime > 60000) {
            if (unclaimOwnership() != null) {
                mapOwnerFirstActivityTime = Long.MAX_VALUE;
                this.dropMessage(6, "超过1分钟没有打怪行为，当前地图为无归属状态");
            }
        }
    }
